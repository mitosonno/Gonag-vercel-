  ...
  return (
    <svg width={total} height={total} style={{display:"block",overflow:"visible"}}>
      {Array.from({length:seats}).map((_,i)=>{
        ...
        const sx=cx+chairR*Math.cos(angle), sy=cy+chairR*Math.sin(angle);
        const f=i<filled;
        const cc=f?"#4ade80":side==="Oğlan evi"?"#93c5fd":side==="Qız evi"?"#f9a8d4":"#fcd34d";
        return <rect key={i} x={sx-chairW/2} y={sy-chairH/2} width={chairW} height={chairH}
          rx={Math.max(1,chairH*0.4)} fill={cc} opacity={f?0.95:0.3}
          transform={`rotate(${(angle*180/Math.PI)+90} ${sx} ${sy})`}/>;
      })}
      <circle cx={cx} cy={cy} r={r-2} fill={fillBg} stroke={sc} strokeWidth="2.5"
        style={{filter:`drop-shadow(0 0 4px ${sc}99)`}}/>
      <text x={cx} y={cy-4} textAnchor="middle" dominantBaseline="middle"
        fontSize={Math.max(10,S*0.24)} fontWeight="800" fill={sc}>{table.id}</text>
      <text x={cx} y={cy+S*0.14} textAnchor="middle"
        fontSize={Math.max(7,S*0.13)} fill="rgba(255,255,255,.45)">{filled}/{seats}</text>
    </svg>
  );
}
          ...
          <>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:14}}>
              ...
              <div style={{display:"flex",gap:6}}>
                {[["👨 Kişi","kishi","#7aade8"],["👩 Qadın","qadin","#e87aad"],["👶 Uşaq","ushaq","#f5d060"]].map(([l,v,c])=>(
                  <button key={v} onClick={()=>setGender(g=>g===v?"":v)}
                    style={{flex:1,padding:"8px 4px",borderRadius:8,border:"1.5px solid "+(gender===v?c:"rgba(255,255,255,.08)"),background:gender===v?c+"22":"transparent",color:gender===v?c:"rgba(255,255,255,.3)",fontSize:11,fontWeight:700,cursor:"pointer"}}>
                    {l}
                  </button>
                ))}
              </div>
              <div style={{display:"flex",gap:8}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:10,color:"rgba(255,255,255,.4)",marginBottom:4}}>Böyük say</div>
                  <div style={{display:"flex",alignItems:"center",gap:8,background:"rgba(255,255,255,.04)",borderRadius:8,padding:"6px 10px"}}>
                    <button onClick={()=>setCount(c=>String(Math.max(1,parseInt(c)-1)))} style={{width:24,height:24,borderRadius:"50%",border:"none",background:"rgba(201,168,76,.2)",color:"#c9a84c",fontSize:16,cursor:"pointer",fontWeight:700}}>−</button>
                    <span style={{flex:1,textAlign:"center",fontSize:16,fontWeight:800,color:"#c9a84c"}}>{count}</span>
                    <button onClick={()=>setCount(c=>String(parseInt(c)+1))} style={{width:24,height:24,borderRadius:"50%",border:"none",background:"rgba(201,168,76,.2)",color:"#c9a84c",fontSize:16,cursor:"pointer",fontWeight:700}}>+</button>
                  </div>
                </div>
                <div style={{flex:1}}>
                  <div style={{fontSize:10,color:"rgba(255,255,255,.4)",marginBottom:4}}>Uşaq say</div>
                  <div style={{display:"flex",alignItems:"center",gap:8,background:"rgba(255,255,255,.04)",borderRadius:8,padding:"6px 10px"}}>
                    <button onClick={()=>setUshaq(c=>String(Math.max(0,parseInt(c)-1)))} style={{width:24,height:24,borderRadius:"50%",border:"none",background:"rgba(245,208,96,.2)",color:"#f5d060",fontSize:16,cursor:"pointer",fontWeight:700}}>−</button>
                    <span style={{flex:1,textAlign:"center",fontSize:16,fontWeight:800,color:"#f5d060"}}>{ushaq}</span>
                    <button onClick={()=>setUshaq(c=>String(parseInt(c)+1))} style={{width:24,height:24,borderRadius:"50%",border:"none",background:"rgba(245,208,96,.2)",color:"#f5d060",fontSize:16,cursor:"pointer",fontWeight:700}}>+</button>
                  </div>
                </div>
              </div>
            </div>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>onSave({...guest,name:name.trim(),phone:phone.trim()?"+994"+phone.trim():"",count:parseInt(count)||1,ushaqCount:parseInt(ushaq)||0,gender})}
                style={{flex:2,padding:11,borderRadius:10,border:"none",background:"rgba(201,168,76,.2)",color:"#c9a84c",fontSize:13,fontWeight:700,cursor:"pointer"}}>✓ Saxla</button>
              <button onClick={()=>setMode("move")}
                style={{flex:1,padding:11,borderRadius:10,border:"1px solid rgba(122,173,232,.3)",background:"rgba(122,173,232,.08)",color:"#7aade8",fontSize:12,cursor:"pointer"}}>↔ Köçür</button>
              <button onClick={()=>onDelete(guest.id)}
                style={{padding:"11px 14px",borderRadius:10,border:"1px solid rgba(255,80,80,.3)",background:"rgba(255,80,80,.08)",color:"#ff8888",fontSize:14,cursor:"pointer"}}>🗑</button>
            </div>
          </>
        ):(
          <>
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>{if(moveTo)onMove(guest.id,tableId,parseInt(moveTo));}} disabled={!moveTo}
                style={{flex:1,padding:11,borderRadius:10,border:"none",background:moveTo?"rgba(122,173,232,.2)":"rgba(255,255,255,.05)",color:moveTo?"#7aade8":"rgba(255,255,255,.2)",fontSize:13,fontWeight:700,cursor:moveTo?"pointer":"default"}}>↔ Köçür</button>
              <button onClick={()=>setMode("edit")}
                style={{padding:"11px 18px",borderRadius:10,border:"none",background:"transparent",color:"rgba(255,255,255,.3)",fontSize:12,cursor:"pointer"}}>Geri</button>
            </div>
          </>
        )}
      </div>
...
export default function InvitePage(){
  ...
  // Yeni qonaq formu
  const [addForm,setAddForm]=useState(null); // {tableId}
  const [fName,setFName]=useState("");
  const [fPhone,setFPhone]=useState("");
  const [fCount,setFCount]=useState("1");
  const [fUshaq,setFUshaq]=useState("0");
  ...
  async function loadInvite(){
    ...
    try{
      ...
      setTables(myTables); setStatus("ready");
  ...
  function addGuest(){
    ...
    setTables(prev=>prev.map(t=>t.id!==tblId?t:{...t,guests:[...t.guests,newG]}));
    setAddForm(null); setFName(""); setFPhone(""); setFCount("1"); setFUshaq("0"); setFGender("");
  }

  function saveGuestEdit(updated){
    setTables(prev=>prev.map(t=>t.id!==editPopup.tableId?t:{...t,guests:t.guests.map(g=>g.id===updated.id?updated:g)}));
    setEditPopup(null);
  }
  function deleteGuest(gId){
    setTables(prev=>prev.map(t=>t.id!==editPopup.tableId?t:{...t,guests:t.guests.filter(g=>g.id!==gId)}));
    setEditPopup(null);
  }
  function moveGuest(gId,fromId,toId){
    setTables(prev=>{
      const g=prev.find(t=>t.id===fromId)?.guests.find(g=>g.id===gId);
      if(!g) return prev;
      return prev.map(t=>{ if(t.id===fromId)return{...t,guests:t.guests.filter(x=>x.id!==gId)}; if(t.id===toId)return{...t,guests:[...t.guests,{...g}]}; return t; });
    });
    setEditPopup(null);
  }
  ...
  if(status==="loading") return (
    <div style={{minHeight:"100vh",background:"#080604",display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{textAlign:"center"}}><div style={{fontSize:36,marginBottom:12}}>🎊</div><div style={{color:"#c9a84c",fontSize:14,fontWeight:600}}>Yüklənir...</div></div>
    </div>
  );
  if(status==="error") return (
    <div style={{minHeight:"100vh",background:"#080604",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{textAlign:"center"}}><div style={{fontSize:48,marginBottom:16}}>😕</div><div style={{color:"#ff9999",fontSize:16,fontWeight:700,marginBottom:8}}>Link tapılmadı</div><div style={{color:"rgba(255,255,255,.4)",fontSize:13}}>Dəvəti göndərən şəxslə əlaqə saxlayın.</div></div>
    </div>
  );
  ...
  return (
    <div style={{minHeight:"100vh",background:"#080604",fontFamily:"'DM Sans',sans-serif",color:"#f2e8d0"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@700&family=DM+Sans:wght@300;400;500&display=swap');*{box-sizing:border-box;margin:0;padding:0}select option{background:#1a1208;color:#f2e8d0}`}</style>
      ...
      <div style={{maxWidth:500,margin:"0 auto",padding:"16px 14px 80px"}}>
        {tables.map(t=>{
            ...
            <div key={t.id} style={{background:"rgba(255,255,255,.02)",border:`1px solid ${isFull?"rgba(80,200,120,.4)":"rgba(201,168,76,.15)"}`,borderRadius:16,marginBottom:20,transition:"border-color .4s"}}>
                ...
                {t.guests.map((g,gi)=>{
                  ...
                  return (
                    <div key={g.id} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 0",borderBottom:gi<t.guests.length-1?"1px solid rgba(255,255,255,.05)":"none"}}>
                      <div style={{width:32,height:32,borderRadius:"50%",flexShrink:0,background:gSc+"22",border:"1px solid "+gSc+"44",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:700,color:gSc}}>
                        {g.name[0]||"?"}
                      </div>
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{fontSize:10,color:"rgba(255,255,255,.3)"}}>
                          {g.count||1} böyük{g.ushaqCount>0?` + ${g.ushaqCount} uşaq`:""}
                          {g.phone&&` · ${g.phone}`}
                        </div>
                      </div>
                      <button onClick={()=>setEditPopup({guest:g,tableId:t.id})}
                        style={{width:30,height:30,borderRadius:8,flexShrink:0,border:"1px solid rgba(201,168,76,.18)",background:"rgba(201,168,76,.06)",color:"rgba(201,168,76,.7)",fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>✏️</button>
                    </div>
                  );
                })}

                {/* Qonaq əlavə et düyməsi */}
                {filled<seats&&(
                  <button onClick={()=>setAddForm({tableId:t.id})}
                    style={{width:"100%",marginTop:10,padding:"9px",borderRadius:9,border:"1px dashed rgba(201,168,76,.3)",background:"transparent",color:"rgba(201,168,76,.6)",fontSize:12,fontWeight:600,cursor:"pointer"}}>
                    + Qonaq əlavə et
                  </button>
                )}
              </div>
              ...
              <div style={{padding:"8px 14px 14px"}}>
                <button onClick={()=>saveTable(t.id)} disabled={isSaving||t.guests.length===0}
                  style={{width:"100%",padding:"11px",borderRadius:10,border:"none",background:isSaved?"rgba(80,200,120,.18)":t.guests.length===0?"rgba(255,255,255,.03)":"rgba(201,168,76,.16)",color:isSaved?"#50c878":t.guests.length===0?"rgba(255,255,255,.18)":"#c9a84c",fontSize:12,fontWeight:700,cursor:isSaving||t.guests.length===0?"default":"pointer",transition:"all .3s"}}>
      ...
      {/* Qonaq əlavə etmə formu — aşağıdan sürüşdürülən panel (əsas sxemlə eyni) */}
      {addForm&&(
        <div style={{position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,.6)"}} onClick={()=>setAddForm(null)}>
          <div style={{position:"absolute",bottom:0,left:0,right:0,background:"#0e0a04",borderTop:"1px solid rgba(201,168,76,.3)",borderRadius:"20px 20px 0 0",padding:"20px 16px 36px"}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:"flex",gap:6,marginBottom:8}}>
              {[["👨 Kişi","kishi","#7aade8"],["👩 Qadın","qadin","#e87aad"],["👶 Uşaq","ushaq","#f5d060"]].map(([l,v,c])=>(
                <button key={v} onClick={()=>setFGender(g=>g===v?"":v)}
                  style={{flex:1,padding:"8px 4px",borderRadius:8,border:"1.5px solid "+(fGender===v?c:"rgba(255,255,255,.08)"),background:fGender===v?c+"22":"transparent",color:fGender===v?c:"rgba(255,255,255,.3)",fontSize:11,fontWeight:700,cursor:"pointer"}}>
                  {l}
                </button>
              ))}
            </div>
            ...
            <button onClick={addGuest} disabled={!fName.trim()}
              style={{width:"100%",padding:"13px",borderRadius:10,border:"none",background:fName.trim()?"linear-gradient(90deg,rgba(80,200,120,.5),rgba(80,200,120,.3))":"rgba(255,255,255,.05)",color:fName.trim()?"#50c878":"rgba(255,255,255,.2)",fontSize:13,fontWeight:800,cursor:fName.trim()?"pointer":"default"}}>
              ✅ Əlavə et
            </button>
          </div>
        </div>

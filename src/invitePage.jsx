...
export default function InvitePage({ code }) {
  ...
  useEffect(() => {
    async function load() {
      try {
        // invite_links cədvəlindən code ilə tap
        const rows = await sbFetch("invite_links?code=eq." + encodeURIComponent(code) + "&limit=1");
        ...
        // Yalnız link-də olan masalar
        const tableIds = link.table_ids || [];
        const myTables = allTables.filter(t => tableIds.includes(t.id));
        ...
        setData({
          ...
          hallAddr: meta.obData?.hallAddr || "",
          hallMaps: meta.obData?.hallMaps || "",
          tables: myTables,
          allTables,
          eventId: ev.id,
          meta
  ...
  async function handleSave() {
    ...
    // events cədvəlindəki masaya əlavə et
  ...
  if (loading) return (
    <div style={{minHeight:"100vh",background:bg,display:"flex",alignItems:"center",justifyContent:"center",
      fontFamily:"'DM Sans',sans-serif",color:gold,fontSize:16}}>
      Yüklənir...
    </div>
  );
  ...
  if (error) return (
    <div style={{minHeight:"100vh",background:bg,display:"flex",alignItems:"center",justifyContent:"center",
      fontFamily:"'DM Sans',sans-serif",color:"#ff6666",fontSize:14,textAlign:"center",padding:24}}>
      ⚠️ {error}
    </div>
  );
  ...
  if (saved) return (
    <div style={{minHeight:"100vh",background:bg,display:"flex",flexDirection:"column",
      ...
      <div style={{fontSize:14,color:"rgba(255,255,255,.6)",lineHeight:1.7}}>
        Adınız məclisə əlavə olundu.<br/>
        {data.evName} toyunda görüşərik!
      </div>
      {data.hallAddr && (
        <div style={{marginTop:20,fontSize:13,color:"rgba(201,168,76,.6)"}}>
          📍 {data.hallAddr}
        </div>
      )}
      {data.hallMaps && (
        <a href={data.hallMaps} target="_blank" rel="noreferrer"
          style={{marginTop:8,display:"inline-block",padding:"8px 16px",borderRadius:10,
  ...
  return (
    <div style={{minHeight:"100vh",background:bg,fontFamily:"'DM Sans',sans-serif",
      color:"#f2e8d0",padding:"0 0 40px"}}>
      
      {/* Header */}
      <div style={{background:"rgba(201,168,76,.08)",borderBottom:"1px solid rgba(201,168,76,.15)",
        padding:"16px 20px",textAlign:"center"}}>
        <div style={{fontFamily:"Georgia,serif",fontSize:22,color:gold,letterSpacing:3,marginBottom:4}}>
          GONAG.AZ
        </div>
        <div style={{fontFamily:"Georgia,serif",fontSize:18,color:"#f2e8d0",marginBottom:4}}>
          🎊 {data.evName}
        </div>
      ...
      <div style={{padding:"20px 16px",maxWidth:480,margin:"0 auto"}}>
          ...
          {data.tables.map(t => {
            ...
            return (
              <div key={t.id}
                ...
                style={{
                  background: isSel ? sc + "18" : "rgba(255,255,255,.04)",
                  ...
                  <div style={{marginTop:10,paddingTop:10,borderTop:"1px solid rgba(255,255,255,.06)"}}>
                    ...
                    {t.guests.map((g, i) => (
                      <div key={i} style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                        ...
                        {g.count > 1 && <span style={{fontSize:10,color:"rgba(255,255,255,.3)"}}>×{g.count}</span>}
                      </div>
                    ))}
                  </div>
                )}
          ...
          <div style={{background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.2)",
            ...
            <input
              ...
              style={{display:"block",width:"100%",boxSizing:"border-box",
                ...
                borderRadius:10,color:"#f2e8d0",fontSize:14,outline:"none",
            ...
            <div style={{display:"flex",alignItems:"center",background:"rgba(255,255,255,.06)",
              ...
              <input
                ...
                style={{flex:1,padding:"11px 8px",background:"transparent",border:"none",
                  color:"#f2e8d0",fontSize:13,outline:"none",fontFamily:"'DM Sans',sans-serif"}}
              />
            </div>
            <button
              onClick={handleSave}
        ...
        {/* Ünvan */}
        {(data.hallAddr || data.hallMaps) && (
          <div style={{marginTop:20,background:"rgba(255,255,255,.04)",
            border:"1px solid rgba(201,168,76,.15)",borderRadius:12,padding:"14px 16px"}}>
            <div style={{fontSize:12,color:"rgba(201,168,76,.6)",fontWeight:600,marginBottom:8}}>🏛️ Məclis yeri</div>
            {data.hallName && <div style={{fontSize:13,color:"#f2e8d0",marginBottom:4}}>{data.hallName}</div>}
            {data.hallAddr && <div style={{fontSize:12,color:"rgba(255,255,255,.5)",marginBottom:8}}>📍 {data.hallAddr}</div>}
            {data.hallMaps && (
              <a href={data.hallMaps} target="_blank" rel="noreferrer"
                style={{display:"inline-block",padding:"7px 14px",borderRadius:8,
                  border:"1px solid rgba(201,168,76,.3)",background:"rgba(201,168,76,.1)",
                  color:gold,fontSize:12,textDecoration:"none"}}>
                🗺️ Xəritədə göstər
              </a>

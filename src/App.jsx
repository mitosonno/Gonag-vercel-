              ...
              <button onClick={function(){
                ...
                try{
                  fetch("https://dpvoluttxelwnqcfnsbh.supabase.co/rest/v1/invite_links",{
                    ...
                    var evDbId=(savedEventsRef.current.find(function(e){return e.id===currentEvId;})||{}).dbId||null;
                    body:JSON.stringify({code:code,event_id:evDbId?String(evDbId):null,session_id:"gonag_user_main",table_ids:tblIds,status:"active"})
                  }).catch(function(){});
                }catch(e){}
...
function MasaDevetCard({ tbl, ev, hall }){
    ...
    <div style={{background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.2)",borderRadius:14,padding:"12px 14px",marginTop:8,marginBottom:4}}>
      ...
      {!shareOpen?(
        <button onClick={()=>setShareOpen(true)}
          style={{width:"100%",padding:"9px",borderRadius:10,border:"none",background:"linear-gradient(90deg,rgba(37,211,102,.3),rgba(37,211,102,.15))",color:"#25d366",fontSize:13,fontWeight:700,cursor:"pointer"}}>
          📤 Dəvəti Göndər
        </button>
      ):(
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
            ...
            {msgs.map((m,i)=>(
              <div key={i} className={"mw "+m.role}>
                ...
                <div>
                  ...
                  {m.masaCard&&(
                    <MasaDevetCard tbl={m.masaCard} ev={m.ev} hall={m.hall}/>
                  )}
                  {m.masaCards&&m.masaCards.map((t,ti)=>(
                    <MasaDevetCard key={ti} tbl={t} ev={m.ev} hall={m.hall}/>
                  ))}

...
// ─── DƏVƏTNAMƏ PANELİ ────────────────────────────────────
function DevetNamePanel({ obData, hall, tables, initialTab, devetData, onSave }){
  const gold="#c9a84c";
  const [tab, setTab] = useState("oz"); // "oz"|"shablon"|"masa"
  const [metn, setMetn] = useState((devetData&&devetData.metn)||"");
  const [media, setMedia] = useState((devetData&&devetData.media)||null);
  const [hallAddr, setHallAddr] = useState((devetData&&devetData.hallAddr)||(hall&&hall.address)||"");
  const [hallMaps, setHallMaps] = useState((devetData&&devetData.hallMaps)||"");
  const [saved, setSaved] = useState(false);
  const [selectedShablon, setSelectedShablon] = useState(null);

  const evName = (obData&&obData.boy&&obData.girl)
    ? (obData.boy+" & "+obData.girl)
    : ((obData&&obData.name)||(obData&&obData.company)||"Məclis");
  const evDate = (obData&&obData.date)||"";
  const hallName = (hall&&hall.name)||"";

  const defaultMetn = "Hörmətli [Ad],\n\n"
  ...
  useEffect(function(){
    if(!metn) setMetn(defaultMetn);
  },[evName,evDate,hallName]);

  // Hazır şablonlar
  const SHABLONLAR = [
    {id:1, emoji:"💍", ad:"Klassik Toy", color:"#c9a84c",
     metn:"Hörmətli [Ad],\n\nSizi [EVNAME] toyuna dəvət etməkdən böyük şərəf duyuruq!\n📅 [TARİX]\n📍 [ZAL]\n🪑 Masa № [Masa]\n\nBizimlə bu xoşbəxt günü paylaşmağınızı ümid edirik! 💫\n\nGONAG.AZ"},
    {id:2, emoji:"🌹", ad:"Romantik", color:"#e87aad",
     metn:"Əziz [Ad],\n\nXoşbəxtliyimizin şahidi olmanızı arzulayırıq!\n\n[EVNAME]\n📅 [TARİX]\n📍 [ZAL]\n🌹 Masa № [Masa]\n\nSizi gözləyirik! ❤️\n\nGONAG.AZ"},
    {id:3, emoji:"✨", ad:"Müasir", color:"#7aade8",
     metn:"Hi [Ad]! 👋\n\n[EVNAME] toyumuza dəvətlisən!\n📅 [TARİX] | 📍 [ZAL]\n🪑 Masa [Masa]\n\nSəni görmək üçün səbrsizliklə gözləyirik! 🎉\n\nGONAG.AZ"},
    {id:4, emoji:"🏛️", ad:"Rəsmi", color:"#b57aff",
     metn:"Hörmətli [Ad] xanım/bəy,\n\n[EVNAME] cənablarının toy mərasimlərinə Sizi hörmətlə dəvət edirik.\n\nTarix: [TARİX]\nYer: [ZAL]\nMasa: № [Masa]\n\nHörmətlə,\nAilə\n\nGONAG.AZ"},
  ];

  function applyMetn(shablon){
    const t=shablon.metn
      .replace(/[EVNAME]/g,evName)
      .replace(/[TARİX]/g,evDate)
      .replace(/[ZAL]/g,hallName||"Zal")
      .replace(/[Ad]/g,"[Ad]")
      .replace(/[Masa]/g,"[Masa]");
    setMetn(t);
    setSelectedShablon(shablon.id);
    setTab("oz");
    setSaved(false);
  }
  ...
  function handleMedia(e,type){
    ...
    reader.onload=function(ev){
      setMedia({type:type,url:ev.target.result,name:file.name});
      setSaved(false);
    };
  ...
  function handleSave(){
    if(onSave) onSave({metn:metn,media:media,hallName:hallName,hallAddr:hallAddr,hallMaps:hallMaps});
    setSaved(true);
  }

  // Masa məlumatları mətni
  const tblCount=tables?tables.length:0;
  const totG=tables?tables.reduce(function(s,t){return s+t.guests.reduce(function(ss,g){return ss+(g.count||1);},0);},0):0;

  return (
    <div>
      {/* Saxlanıb banner */}
      {devetData&&devetData.metn&&(
        <div style={{background:"rgba(80,200,120,.08)",border:"1px solid rgba(80,200,120,.25)",
          borderRadius:10,padding:"8px 12px",marginBottom:12,display:"flex",alignItems:"center",gap:8}}>
          <span>✅</span>
          <div style={{flex:1}}>
            <div style={{fontSize:11,color:"#50c878",fontWeight:700}}>Dəvətnamə saxlanılıb</div>
            <div style={{fontSize:9,color:"rgba(80,200,120,.5)"}}>
              {devetData.media?"📎 "+devetData.media.name+" · ":""}Göndərilməyən bölməsindən göndərin
            </div>
          </div>
          <button onClick={function(){setSaved(false);}}
            style={{fontSize:10,color:"rgba(201,168,76,.6)",background:"none",border:"none",cursor:"pointer"}}>
            ✏️ Dəyiş
          </button>
        </div>
      )}
      ...
      {/* 3 Tab */}
      <div style={{display:"flex",gap:4,marginBottom:14,background:"rgba(255,255,255,.04)",borderRadius:12,padding:4}}>
        {[["oz","📤 Öz materialım"],["shablon","✨ Hazır şablonlar"],["masa","🗺️ Masa məlumatı"]].map(function(item){
          var t=item[0],lbl=item[1];
          return (
            <button key={t} onClick={function(){setTab(t);}}
              style={{flex:1,padding:"8px 2px",borderRadius:9,fontSize:10,fontWeight:700,
                cursor:"pointer",border:"none",
                background:tab===t?"rgba(201,168,76,.2)":"transparent",
                color:tab===t?gold:"rgba(201,168,76,.4)"}}>
              {lbl}
            </button>
          );
        })}
      ...
      {tab==="oz"&&(
        <div>
          {/* Mətn */}
          <div style={{marginBottom:12}}>
            <div style={{fontSize:10,color:"rgba(201,168,76,.6)",marginBottom:6,fontWeight:600}}>
              📝 Dəvət mətni
              <span style={{opacity:.6,fontWeight:400}}> — [Ad] və [Masa] avtomatik doldurulur</span>
            </div>
            <textarea value={metn} onChange={function(e){setMetn(e.target.value);setSaved(false);}} rows={7}
              style={{width:"100%",boxSizing:"border-box",padding:"10px 12px",
                background:"rgba(255,255,255,.06)",border:"1px solid rgba(201,168,76,.3)",
                borderRadius:10,color:"#f2e8d0",fontSize:12,outline:"none",
                fontFamily:"'DM Sans',sans-serif",lineHeight:1.6,resize:"vertical"}}/>
            {selectedShablon&&(
              <div style={{fontSize:9,color:"rgba(201,168,76,.5)",marginTop:3}}>
                ✨ {SHABLONLAR.find(function(s){return s.id===selectedShablon;})&&SHABLONLAR.find(function(s){return s.id===selectedShablon;}).ad} şablonundan
              </div>
            )}
          </div>
          ...
          {/* Şəkil/Video */}
          <div style={{marginBottom:12}}>
            <div style={{fontSize:10,color:"rgba(201,168,76,.6)",marginBottom:8,fontWeight:600}}>📎 Şəkil və ya video</div>
            {media?(
              <div style={{background:"rgba(255,255,255,.05)",border:"1px solid rgba(201,168,76,.2)",
                borderRadius:10,padding:"10px 12px",display:"flex",alignItems:"center",gap:10}}>
                {media.type==="image"?(
                  <img src={media.url} style={{width:48,height:48,objectFit:"cover",borderRadius:8,border:"1px solid rgba(201,168,76,.3)"}}/>
                ):(
                  <div style={{width:48,height:48,background:"rgba(201,168,76,.1)",borderRadius:8,
                    display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>🎥</div>
                )}
                <div style={{flex:1}}>
                  <div style={{fontSize:11,color:"#f2e8d0",fontWeight:600}}>{media.name}</div>
                  <div style={{fontSize:9,color:"rgba(201,168,76,.5)",marginTop:2}}>
                    {media.type==="image"?"Şəkil":"Video"} — göndərəndə mətnlə birlikdə gedir
                  </div>
                </div>
                <button onClick={function(){setMedia(null);setSaved(false);}}
                  style={{background:"none",border:"none",color:"rgba(220,80,80,.6)",fontSize:13,cursor:"pointer"}}>✕</button>
              </div>
            ):(
              <div style={{display:"flex",gap:8}}>
                <label style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",
                  padding:"16px 10px",borderRadius:10,border:"1.5px dashed rgba(201,168,76,.3)",
                  background:"rgba(201,168,76,.04)",cursor:"pointer"}}>
                  <span style={{fontSize:24,marginBottom:4}}>🖼️</span>
                  <span style={{fontSize:10,color:"rgba(201,168,76,.6)",fontWeight:600}}>Şəkil</span>
                  <input type="file" accept="image/*" onChange={function(e){handleMedia(e,"image");}} style={{display:"none"}}/>
                </label>
                <label style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",
                  padding:"16px 10px",borderRadius:10,border:"1.5px dashed rgba(201,168,76,.3)",
                  background:"rgba(201,168,76,.04)",cursor:"pointer"}}>
                  <span style={{fontSize:24,marginBottom:4}}>🎥</span>
                  <span style={{fontSize:10,color:"rgba(201,168,76,.6)",fontWeight:600}}>Video</span>
                  <input type="file" accept="video/*" onChange={function(e){handleMedia(e,"video");}} style={{display:"none"}}/>
                </label>
              </div>
            )}
          </div>
      ...
      {tab==="shablon"&&(
        <div>
          <div style={{fontSize:10,color:"rgba(201,168,76,.5)",marginBottom:10,fontWeight:600}}>
            GONAG.AZ tərəfindən hazır dəvət mətnləri — seç, öz adınla göndər
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {SHABLONLAR.map(function(s){
              return (
                <div key={s.id}
                  style={{background:"rgba(255,255,255,.04)",border:"1px solid "+(selectedShablon===s.id?s.color:s.color+"44"),
                    borderRadius:12,padding:"12px 14px",cursor:"pointer",
                    background:selectedShablon===s.id?s.color+"18":"rgba(255,255,255,.04)"}}
                  onClick={function(){applyMetn(s);}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                    <span style={{fontSize:20}}>{s.emoji}</span>
                    <div>
                      <div style={{fontSize:12,fontWeight:700,color:s.color}}>{s.ad}</div>
                      {selectedShablon===s.id&&<div style={{fontSize:9,color:s.color,opacity:.7}}>✓ Seçildi — Öz materialım tabına keçdi</div>}
                    </div>
                    {selectedShablon!==s.id&&(
                      <button style={{marginLeft:"auto",padding:"4px 10px",borderRadius:8,border:"1px solid "+s.color+"55",
                        background:s.color+"15",color:s.color,fontSize:10,fontWeight:700,cursor:"pointer"}}>
                        Seç
                      </button>
                    )}
                  </div>
                  <div style={{fontSize:10,color:"rgba(255,255,255,.4)",lineHeight:1.5,
                    ...
                    {s.metn.replace(/[EVNAME]/g,evName).replace(/[TARİX]/g,evDate).replace(/[ZAL]/g,hallName).substring(0,120)}...
                  </div>
                </div>
              );
            })}
          </div>
          <div style={{marginTop:12,fontSize:10,color:"rgba(201,168,76,.3)",textAlign:"center"}}>
            🔜 Tezliklə: Hazır dəvət videoları əlavə olunacaq
          </div>
        </div>
      )}
      ...
      {tab==="masa"&&(
        <div>
          <div style={{fontSize:10,color:"rgba(201,168,76,.5)",marginBottom:10,fontWeight:600}}>
            Göndərəndə qonağın mesajına avtomatik əlavə olunacaq
          </div>
          {/* İcmal */}
          <div style={{background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.2)",
            borderRadius:12,padding:"12px 14px",marginBottom:12}}>
            <div style={{fontSize:11,color:"#c9a84c",fontWeight:700,marginBottom:8}}>📊 Məclis icmalı</div>
            <div style={{display:"flex",gap:12}}>
              <div style={{textAlign:"center"}}>
                <div style={{fontSize:20,fontWeight:800,color:"#c9a84c"}}>{tblCount}</div>
                <div style={{fontSize:9,color:"rgba(201,168,76,.5)"}}>Masa</div>
              </div>
              <div style={{textAlign:"center"}}>
                <div style={{fontSize:20,fontWeight:800,color:"#50c878"}}>{totG}</div>
                <div style={{fontSize:9,color:"rgba(80,200,120,.5)"}}>Qonaq</div>
              </div>
              {evDate&&<div style={{textAlign:"center"}}>
                <div style={{fontSize:13,fontWeight:700,color:"#f2e8d0"}}>{evDate}</div>
                <div style={{fontSize:9,color:"rgba(255,255,255,.4)"}}>Tarix</div>
              </div>}
            </div>
          </div>
          ...
          <div style={{marginBottom:10}}>
            <div style={{fontSize:10,color:"rgba(201,168,76,.6)",marginBottom:6,fontWeight:600}}>🏛️ Şadlıq sarayı</div>
            {hallName&&<div style={{fontSize:11,color:"rgba(201,168,76,.7)",marginBottom:6,padding:"6px 10px",background:"rgba(201,168,76,.08)",borderRadius:8}}>{"Zal: "+hallName}</div>}
            <input value={hallAddr} onChange={function(e){setHallAddr(e.target.value);setSaved(false);}}
              placeholder="Ünvan (məs: Neftçilər pr. 2, Bakı)"
              style={{display:"block",width:"100%",boxSizing:"border-box",padding:"7px 10px",marginBottom:7,
                background:"rgba(255,255,255,.06)",border:"1px solid rgba(201,168,76,.2)",
                borderRadius:8,color:"#f2e8d0",fontSize:12,outline:"none",fontFamily:"'DM Sans',sans-serif"}}/>
            <input value={hallMaps} onChange={function(e){setHallMaps(e.target.value);setSaved(false);}}
              placeholder="Google Maps linki"
              style={{display:"block",width:"100%",boxSizing:"border-box",padding:"7px 10px",
                background:"rgba(255,255,255,.06)",border:"1px solid rgba(201,168,76,.2)",
                borderRadius:8,color:"#f2e8d0",fontSize:12,outline:"none",fontFamily:"'DM Sans',sans-serif"}}/>
          </div>
          ...
          <div style={{background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.08)",borderRadius:10,padding:"10px 12px"}}>
            <div style={{fontSize:9,color:"rgba(201,168,76,.5)",marginBottom:6,fontWeight:600}}>📱 NÜMUNƏ WHATSAPP MESAJI:</div>
            <div style={{fontSize:10,color:"rgba(255,255,255,.5)",lineHeight:1.6,whiteSpace:"pre-wrap"}}>
              {"🎊 Dəvətnamə\n━━━━━━━━━━\n\nHörmətli *[Ad]*,\n\n"
              +evName+" mərasiminə dəvət olunursunuz!\n"
              +(evDate?"📅 "+evDate+"\n":"")
              +(hallName?"📍 "+hallName+"\n":"")
      ...
      <div style={{marginTop:14}}>
        <button onClick={handleSave}
          style={{width:"100%",padding:"13px",borderRadius:12,border:"none",
            background:saved
              ?"linear-gradient(90deg,rgba(80,200,120,.4),rgba(80,200,120,.2))"
              :"linear-gradient(90deg,rgba(201,168,76,.45),rgba(201,168,76,.25))",
            color:saved?"#50c878":"#c9a84c",
            fontSize:13,fontWeight:800,cursor:"pointer"}}>
          {saved?"✅ Saxlanıldı — ⏳ Göndərilməyən bölməsindən göndərin":"💾 Dəvətnaməni Saxla"}
        </button>
        {!saved&&(
          <div style={{fontSize:10,color:"rgba(201,168,76,.35)",textAlign:"center",marginTop:6}}>
            Saxladıqdan sonra ⏳ Göndərilməyən bölməsindən göndərin
          </div>
        )}
      </div>
    </div>
...
function InvitePanel({ tables, ev, hall, onRsvp, onMarkInvited }){
  const [mode, setMode] = useState("masa");
  ...
  const allGuests = tables.flatMap(t=>
    t.guests.map(g=>({...g, tblId:t.id, tblLabel:t.label&&t.label!=="__extra__"?t.label:`Masa ${t.id}`, tblNum:t.id, tblSide:t.side||""}))
  );
  const evName = ev?.obData?.boy&&ev?.obData?.girl ? ev.obData.boy+" & "+ev.obData.girl : ev?.obData?.name||ev?.obData?.company||"Məclis";
  const evDate = ev?.obData?.date||"";
  const hallName = hall?.name||"";
  const hasSides = allGuests.some(g=>g.side==="Oğlan evi"||g.side==="Qız evi");

  // Göndərilən/göndərilməyən
  const invitedAll = allGuests.filter(g=>g.invited);
  const notInvited = allGuests.filter(g=>!g.invited);
  const notInvitedTables = tables.filter(t=>t.guests.some(g=>!g.invited)&&t.guests.length>0);

  function markInvited(guests){
    if(onMarkInvited) onMarkInvited(guests.map(g=>g.id));
  }

  function buildMsg(g){
    return `Hörmətli ${g.name},\n\n${evName} mərasiminə dəvət olunursunuz!\n📅 ${evDate}${hallName?"\n📍 "+hallName:""}\n🪑 Masa № ${g.tblNum}${g.tblLabel&&g.tblLabel!==String(g.tblNum)?" — "+g.tblLabel:""}\n\nXoş gəlmisiniz! 🥂\n\nGONAG.AZ`;
  }

  function sendWhatsApp(g){
    const num=(g.phone||"").replace(/\D/g,"");
    if(!num){ alert(g.name+" — nömrə yoxdur!"); return; }
    window.open(`https://wa.me/${num}?text=${encodeURIComponent(buildMsg(g))}`,"_blank");
    markInvited([g]);
  }

  function sendAll(guests){
    const withPhone=guests.filter(g=>(g.phone||"").replace(/\D/g,"").length>=7);
    const noPhone=guests.filter(g=>!((g.phone||"").replace(/\D/g,"").length>=7));
    if(withPhone.length===0){ alert("Seçilən qonaqların nömrəsi yoxdur!"); return; }
    withPhone.forEach((g,i)=>{
      setTimeout(()=>{
        window.open(`https://wa.me/${(g.phone||"").replace(/\D/g,"")}?text=${encodeURIComponent(buildMsg(g))}`,"_blank");
        // Hər göndərişdə dərhal mark et
        if(onMarkInvited) onMarkInvited([g.id]);
      }, i*600);
    });
    if(noPhone.length>0){
      setTimeout(()=>alert(`${noPhone.length} qonağın nömrəsi yoxdur:\n${noPhone.map(g=>g.name).join(", ")}`), withPhone.length*600+300);
    }
    setConfirmSend(false);
  }

  function downloadPDF(guests){
    const rows=guests.map(g=>`<tr><td>${g.name}</td><td style='text-align:center;font-weight:700;font-size:16px;color:#8B6914'>${g.tblNum}</td><td>${g.tblLabel}</td><td>${g.phone||"—"}</td><td style='text-align:center'>${g.count||1}</td></tr>`).join("");
    const st="style";
    const css="body{font-family:Arial,sans-serif;padding:20px;color:#1a1208;background:#fffef8}h1{color:#8B6914;font-size:20px;margin-bottom:4px}.sub{color:#999;font-size:12px;margin-bottom:16px}table{width:100%;border-collapse:collapse;font-size:12px}th{padding:8px 10px;text-align:left;font-weight:700;color:#8B6914;border-bottom:2px solid #c9a84c;background:#f5edd0}td{padding:7px 10px;border-bottom:1px solid #e0d4b0}tr:nth-child(even){background:#faf7ee}.footer{margin-top:16px;font-size:10px;color:#bbb;text-align:center}";
    const html=`<!DOCTYPE html><html><head><meta charset='utf-8'><title>${evName}</title><${st}>${css}</${st}></head><body><h1>📋 ${evName}</h1><div class='sub'>${evDate}${hallName?" · "+hallName:""} · ${guests.length} qonaq</div><table><thead><tr><th>Ad Soyad</th><th>Masa</th><th>Masa adı</th><th>Telefon</th><th>Nəfər</th></tr></thead><tbody>${rows}</tbody></table><div class='footer'>GONAG.AZ</div></body></html>`;
  ...
  return (
    <div>
      {/* İCMAL BANNERI */}
      <div style={{background:"rgba(20,14,4,.9)",border:"1px solid rgba(201,168,76,.25)",borderRadius:12,padding:"12px 14px",marginBottom:14}}>
        <div style={{fontSize:11,fontWeight:700,color:"rgba(201,168,76,.6)",letterSpacing:1,marginBottom:8}}>DƏVƏT VƏZİYYƏTİ</div>
        <div style={{display:"flex",gap:10,marginBottom:notInvitedTables.length>0?10:0}}>
          <div style={{flex:1,textAlign:"center"}}>
            <div style={{fontSize:20,fontWeight:800,color:"#50c878"}}>{invitedAll.length}</div>
            <div style={{fontSize:9,color:"rgba(80,200,120,.6)"}}>✓ Göndərildi</div>
          </div>
          <div style={{flex:1,textAlign:"center"}}>
            <div style={{fontSize:20,fontWeight:800,color:"#e8b87a"}}>{notInvited.length}</div>
            <div style={{fontSize:9,color:"rgba(232,184,122,.6)"}}>⏳ Göndərilməyib</div>
          </div>
          <div style={{flex:1,textAlign:"center"}}>
            <div style={{fontSize:20,fontWeight:800,color:gold}}>{allGuests.length}</div>
            <div style={{fontSize:9,color:"rgba(201,168,76,.6)"}}>👥 Cəmi</div>
          </div>
        </div>
        {notInvitedTables.length>0&&(
          <div style={{borderTop:"1px solid rgba(201,168,76,.12)",paddingTop:8}}>
            <div style={{fontSize:10,color:"#e8b87a",marginBottom:5}}>⏳ Dəvət göndərilməyən masalar:</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
              {notInvitedTables.map(t=>{
                const sc=t.side==="Oğlan evi"?"#7aade8":t.side==="Qız evi"?"#e87aad":gold;
                const remaining=t.guests.filter(g=>!g.invited).length;
                return (
                  <span key={t.id} style={{padding:"2px 8px",borderRadius:20,fontSize:10,fontWeight:700,
        ...
        {notInvited.length===0&&allGuests.length>0&&(
          <div style={{fontSize:11,color:"#50c878",fontWeight:700,textAlign:"center",marginTop:4}}>✅ Bütün qonaqlara dəvət göndərildi!</div>
        )}
      </div>

      {/* 3 MOD */}
      <div style={{display:"flex",gap:6,marginBottom:14}}>
        {[["masa","🪑 Masa üzrə"],["hamisi","📨 Hamısına"],["teref","👥 Tərəf üzrə"]].map(([m,lbl])=>(
          <button key={m} onClick={()=>{setMode(m);setSelectedTbl(null);setSelectedSide(null);setConfirmSend(false);}}
            style={{flex:1,padding:"8px 4px",borderRadius:9,fontSize:10,fontWeight:700,cursor:"pointer",
              border:"1.5px solid "+(mode===m?gold:gold+"33"),
              background:mode===m?"rgba(201,168,76,.15)":"rgba(201,168,76,.04)",
              color:mode===m?gold:"rgba(201,168,76,.5)"}}>
            {lbl}
          </button>
        ))}
      </div>
      ...
      {mode==="masa"&&(
        <div>
          <div style={{fontSize:10,color:"rgba(201,168,76,.5)",marginBottom:8,fontWeight:600}}>Masa seçin:</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
            {tables.filter(t=>t.guests.length>0).map(t=>{
              const sc=t.side==="Oğlan evi"?"#7aade8":t.side==="Qız evi"?"#e87aad":gold;
              const sentCount=t.guests.filter(g=>g.invited).length;
              const allSent=sentCount===t.guests.length;
              return (
                <button key={t.id} onClick={()=>setSelectedTbl(selectedTbl===t.id?null:t.id)}
                  style={{padding:"7px 10px",borderRadius:9,fontSize:11,fontWeight:700,cursor:"pointer",
                    border:"1.5px solid "+(selectedTbl===t.id?sc:allSent?"rgba(80,200,120,.4)":sc+"44"),
                    background:selectedTbl===t.id?sc+"22":allSent?"rgba(80,200,120,.08)":"rgba(255,255,255,.04)",
                    color:selectedTbl===t.id?sc:allSent?"#50c878":sc+"88",
                    opacity:allSent?0.7:1}}>
                  {allSent?"✓ ":""}№{t.id}{t.label&&t.label!=="__extra__"?" · "+t.label:""}
                  <span style={{fontSize:9,opacity:.6}}> ({t.guests.length}n)</span>
                </button>
              );
            })}
          </div>
          ...
          {selectedTbl&&(()=>{
            const guests=allGuests.filter(g=>g.tblId===selectedTbl);
            const notSent=guests.filter(g=>!g.invited);
            return (
              <div>
                {notSent.length>0&&(
                  <div style={{display:"flex",gap:8,marginBottom:10}}>
                    <button onClick={()=>{if(notSent.filter(g=>g.phone).length>0)setConfirmSend("masa");else alert("Bu masada nömrəsi olan göndərilməmiş qonaq yoxdur!");}}
                      style={{flex:1,padding:"10px",borderRadius:10,border:"none",background:"linear-gradient(90deg,rgba(37,211,102,.35),rgba(37,211,102,.2))",color:"#25d366",fontSize:12,fontWeight:700,cursor:"pointer"}}>
                      📱 Göndərilməyənlərə göndər ({notSent.filter(g=>g.phone).length}n)
                    </button>
                    <button onClick={()=>downloadPDF(guests)}
                      style={{padding:"10px 12px",borderRadius:10,border:"1px solid rgba(201,168,76,.3)",background:"rgba(201,168,76,.08)",color:gold,fontSize:12,cursor:"pointer"}}>📄</button>
                  </div>
                )}
                {notSent.length===0&&<div style={{fontSize:11,color:"#50c878",textAlign:"center",padding:"8px 0",marginBottom:8}}>✅ Bu masanın bütün qonaqlarına dəvət göndərildi</div>}
                {confirmSend==="masa"&&(
                  <div style={{background:"rgba(37,211,102,.08)",border:"1px solid rgba(37,211,102,.3)",borderRadius:10,padding:"12px",marginBottom:10}}>
                    <div style={{fontSize:12,color:"#78e896",marginBottom:8}}>Masa №{selectedTbl} — {notSent.filter(g=>g.phone).length} nəfərə göndəriləcək. Təsdiq?</div>
                    <div style={{display:"flex",gap:6}}>
                      <button onClick={()=>sendAll(notSent.filter(g=>g.phone))} style={{flex:1,padding:"8px",borderRadius:8,border:"none",background:"rgba(37,211,102,.3)",color:"#78e896",fontSize:12,fontWeight:700,cursor:"pointer"}}>✓ Göndər</button>
                      <button onClick={()=>setConfirmSend(false)} style={{padding:"8px 12px",borderRadius:8,border:"1px solid rgba(255,255,255,.1)",background:"transparent",color:"#9a8060",fontSize:12,cursor:"pointer"}}>Ləğv et</button>
                    </div>
                  </div>
                )}
                  ...
                  {guests.map(g=>{
                    ...
                    return (
                      <div key={g.id} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",
                        background:g.invited?"rgba(80,200,120,.06)":"rgba(201,168,76,.04)",
                        border:"1px solid "+(g.invited?"rgba(80,200,120,.25)":"rgba(201,168,76,.1)"),
                        borderRadius:10,opacity:g.invited?0.75:1}}>
                        <div style={{width:30,height:30,borderRadius:"50%",flexShrink:0,background:sc+"22",border:"1px solid "+sc+"44",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:800,color:sc}}>{g.name[0]}</div>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontSize:12,fontWeight:700,color:"#f2e8d0",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{g.name}</div>
                          <div style={{fontSize:9,color:"rgba(201,168,76,.4)"}}>{g.phone||"nömrə yoxdur"}</div>
                        </div>
                        {g.invited?<span style={{fontSize:10,color:"#50c878",flexShrink:0}}>✓ Göndərildi</span>:(
                          <button onClick={()=>sendWhatsApp(g)} disabled={!g.phone}
                            style={{padding:"5px 8px",borderRadius:7,border:"none",background:g.phone?"rgba(37,211,102,.25)":"rgba(255,255,255,.05)",color:g.phone?"#25d366":"rgba(255,255,255,.2)",fontSize:10,fontWeight:700,cursor:g.phone?"pointer":"default",flexShrink:0}}>
                            📱
                          </button>
                        )}
                      </div>
      ...
      {mode==="hamisi"&&(
        <div>
          <div style={{background:"rgba(201,168,76,.06)",border:"1px solid rgba(201,168,76,.2)",borderRadius:12,padding:"12px",marginBottom:12}}>
            <div style={{fontSize:12,color:"#f2e8d0",marginBottom:2,fontWeight:600}}>Göndərilməyən: {notInvited.length} nəfər</div>
            <div style={{fontSize:11,color:"rgba(201,168,76,.5)",marginBottom:10}}>Nömrəsi olan: {notInvited.filter(g=>g.phone).length} nəfər</div>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>{if(notInvited.filter(g=>g.phone).length>0)setConfirmSend("hamisi");else alert("Göndərilməmiş nömrəsi olan qonaq yoxdur!");}}
                style={{flex:1,padding:"10px",borderRadius:10,border:"none",background:"linear-gradient(90deg,rgba(37,211,102,.35),rgba(37,211,102,.2))",color:"#25d366",fontSize:12,fontWeight:700,cursor:"pointer"}}>
                📱 Göndərilməyənlərə göndər ({notInvited.filter(g=>g.phone).length}n)
              </button>
              <button onClick={()=>downloadPDF(allGuests)}
                style={{padding:"10px 12px",borderRadius:10,border:"1px solid rgba(201,168,76,.3)",background:"rgba(201,168,76,.08)",color:gold,fontSize:12,cursor:"pointer"}}>
                📄 PDF
              </button>
            </div>
          </div>
          {confirmSend==="hamisi"&&(
            <div style={{background:"rgba(37,211,102,.08)",border:"1px solid rgba(37,211,102,.3)",borderRadius:10,padding:"12px",marginBottom:12}}>
              <div style={{fontSize:12,color:"#78e896",marginBottom:4,fontWeight:600}}>{notInvited.filter(g=>g.phone).length} nəfərə WhatsApp göndəriləcək</div>
              <div style={{fontSize:10,color:"rgba(255,255,255,.4)",marginBottom:10}}>Hər pəncərə 0.6 saniyə arayla açılır.</div>
              <div style={{display:"flex",gap:6}}>
                <button onClick={()=>sendAll(notInvited.filter(g=>g.phone))} style={{flex:1,padding:"8px",borderRadius:8,border:"none",background:"rgba(37,211,102,.3)",color:"#78e896",fontSize:12,fontWeight:700,cursor:"pointer"}}>✓ Bəli, göndər</button>
                <button onClick={()=>setConfirmSend(false)} style={{padding:"8px 12px",borderRadius:8,border:"1px solid rgba(255,255,255,.1)",background:"transparent",color:"#9a8060",fontSize:12,cursor:"pointer"}}>Ləğv et</button>
              </div>
            </div>
          )}
          {/* Masa icmalı */}
          <div style={{fontSize:10,color:"rgba(201,168,76,.5)",marginBottom:8,fontWeight:600}}>MASA ÜZRƏ:</div>
          {tables.filter(t=>t.guests.length>0).map(t=>{
            const sc=t.side==="Oğlan evi"?"#7aade8":t.side==="Qız evi"?"#e87aad":gold;
            const sentCount=t.guests.filter(g=>g.invited).length;
            const allSent=sentCount===t.guests.length;
            return (
              <div key={t.id} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",
                background:allSent?"rgba(80,200,120,.05)":"rgba(255,255,255,.03)",
                border:"1px solid "+(allSent?"rgba(80,200,120,.2)":"rgba(201,168,76,.1)"),
                borderRadius:9,marginBottom:5,opacity:allSent?0.7:1}}>
                <span style={{fontSize:11,fontWeight:700,color:allSent?"#50c878":sc,minWidth:28}}>№{t.id}</span>
                <span style={{flex:1,fontSize:11,color:"rgba(255,255,255,.6)"}}>{t.label&&t.label!=="__extra__"?t.label:""}{t.side?<span style={{marginLeft:4,fontSize:9,color:sc}}>({t.side})</span>:""}</span>
                <span style={{fontSize:10,color:allSent?"#50c878":"rgba(232,184,122,.7)",fontWeight:700}}>{sentCount}/{t.guests.length}</span>
                {allSent&&<span style={{fontSize:9,color:"#50c878"}}>✓</span>}
              </div>
            );
          })}
        </div>
      ...
      {mode==="teref"&&(
        <div>
          {!hasSides&&(
            <div style={{textAlign:"center",padding:"20px",color:"rgba(201,168,76,.4)",fontSize:12}}>
              Masalara tərəf təyin edilməyib.<br/>Sxemdə masaları "Oğlan evi" / "Qız evi" kimi işarələyin.
            </div>
          )}
          {hasSides&&(
            <div>
              <div style={{fontSize:10,color:"rgba(201,168,76,.5)",marginBottom:8,fontWeight:600}}>Tərəf seçin:</div>
              <div style={{display:"flex",gap:8,marginBottom:14}}>
                {[["Oğlan evi","#7aade8"],["Qız evi","#e87aad"],["Ümumi","#c9a84c"]].map(([side,sc])=>{
                  const cnt=allGuests.filter(g=>(g.side||g.tblSide||"Ümumi")===side).length;
                  const invCnt=allGuests.filter(g=>(g.side||g.tblSide||"Ümumi")===side&&g.invited).length;
                  if(cnt===0) return null;
                  return (
                    <button key={side} onClick={()=>setSelectedSide(selectedSide===side?null:side)}
                      style={{flex:1,padding:"10px 6px",borderRadius:10,fontSize:10,fontWeight:700,cursor:"pointer",
                        border:"1.5px solid "+(selectedSide===side?sc:sc+"44"),
                        background:selectedSide===side?sc+"22":"rgba(255,255,255,.03)",
                        color:selectedSide===side?sc:sc+"66"}}>
                      {side}<br/>
                      <span style={{fontSize:14,fontWeight:800}}>{cnt}</span><span style={{fontSize:9}}> nəfər</span>
                      {invCnt>0&&<><br/><span style={{fontSize:8,color:"#50c878"}}>✓{invCnt} göndərildi</span></>}
                    </button>
                  );
                })}
              </div>
              ...
              {selectedSide&&(()=>{
                const sideGuests=allGuests.filter(g=>(g.side||g.tblSide||"Ümumi")===selectedSide);
                const notSentSide=sideGuests.filter(g=>!g.invited);
                const sc=selectedSide==="Oğlan evi"?"#7aade8":selectedSide==="Qız evi"?"#e87aad":gold;
                return (
                  <div>
                    {notSentSide.length>0&&(
                      <div style={{display:"flex",gap:8,marginBottom:10}}>
                        <button onClick={()=>{if(notSentSide.filter(g=>g.phone).length>0)setConfirmSend("teref");else alert("Bu tərəfdə göndərilməmiş nömrəsi olan qonaq yoxdur!");}}
                          style={{flex:1,padding:"10px",borderRadius:10,border:"none",background:"linear-gradient(90deg,rgba(37,211,102,.35),rgba(37,211,102,.2))",color:"#25d366",fontSize:12,fontWeight:700,cursor:"pointer"}}>
                          📱 {selectedSide} — Göndərilməyənlərə ({notSentSide.filter(g=>g.phone).length}n)
                        </button>
                        <button onClick={()=>downloadPDF(sideGuests)}
                          style={{padding:"10px 12px",borderRadius:10,border:"1px solid rgba(201,168,76,.3)",background:"rgba(201,168,76,.08)",color:gold,fontSize:12,cursor:"pointer"}}>📄</button>
                      </div>
                    )}
                    {notSentSide.length===0&&<div style={{fontSize:11,color:"#50c878",textAlign:"center",padding:"8px 0",marginBottom:8}}>✅ {selectedSide} — bütün qonaqlara dəvət göndərildi</div>}
                    {confirmSend==="teref"&&(
                      <div style={{background:"rgba(37,211,102,.08)",border:"1px solid rgba(37,211,102,.3)",borderRadius:10,padding:"12px",marginBottom:10}}>
                        <div style={{fontSize:12,color:"#78e896",marginBottom:8}}>{selectedSide} — {notSentSide.filter(g=>g.phone).length} nəfərə göndəriləcək. Təsdiq?</div>
                        <div style={{display:"flex",gap:6}}>
                          <button onClick={()=>sendAll(notSentSide.filter(g=>g.phone))} style={{flex:1,padding:"8px",borderRadius:8,border:"none",background:"rgba(37,211,102,.3)",color:"#78e896",fontSize:12,fontWeight:700,cursor:"pointer"}}>✓ Göndər</button>
                          <button onClick={()=>setConfirmSend(false)} style={{padding:"8px 12px",borderRadius:8,border:"1px solid rgba(255,255,255,.1)",background:"transparent",color:"#9a8060",fontSize:12,cursor:"pointer"}}>Ləğv et</button>
                        </div>
                      </div>
                    )}
                    <div style={{display:"flex",flexDirection:"column",gap:5}}>
                      {sideGuests.map(g=>{
                        return (
                          <div key={g.id} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",
                            background:g.invited?"rgba(80,200,120,.06)":"rgba(201,168,76,.04)",
                            border:"1px solid "+(g.invited?"rgba(80,200,120,.25)":"rgba(201,168,76,.1)"),
                            borderRadius:9,opacity:g.invited?0.7:1}}>
                            <div style={{width:28,height:28,borderRadius:"50%",flexShrink:0,background:sc+"22",border:"1px solid "+sc+"44",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:800,color:sc}}>{g.name[0]}</div>
                            <div style={{flex:1,minWidth:0}}>
                              <div style={{fontSize:11,fontWeight:700,color:"#f2e8d0",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{g.name}</div>
                              <div style={{fontSize:9,color:"rgba(201,168,76,.4)"}}>Masa №{g.tblNum} · {g.phone||"nömrə yoxdur"}</div>
                            </div>
                            {g.invited?<span style={{fontSize:9,color:"#50c878",flexShrink:0}}>✓ Göndərildi</span>:(
                              <button onClick={()=>sendWhatsApp(g)} disabled={!g.phone}
                                style={{padding:"4px 7px",borderRadius:6,border:"none",background:g.phone?"rgba(37,211,102,.25)":"rgba(255,255,255,.05)",color:g.phone?"#25d366":"rgba(255,255,255,.2)",fontSize:10,fontWeight:700,cursor:g.phone?"pointer":"default",flexShrink:0}}>
                                📱
                              </button>
                            )}
                          </div>
...
function DevetnamePNGPanel({ tbl, obData, hallName, onClose }){
  ...
  useEffect(()=>{
    if(canvasRef.current){
      drawDevetnamePNG({
        ...
        shablon, tbl, obData, hallName,
        guestName: previewGuest ? previewGuest.name : ""
      });
    }
  },[shablon, selIdx, tbl, obData, hallName]);

  // Bir qonaq üçün PNG blob yarat
  function makeCanvas(gName){
    const c = document.createElement("canvas");
    drawDevetnamePNG({canvas:c, shablon, tbl, obData, hallName, guestName:gName});
    return c;
  }

  // Seçilmiş qonağın PNG-ni endir
  function downloadOne(g){
    const c = makeCanvas(g.name);
    const a = document.createElement("a");
    a.download = "devetname-masa"+tbl.id+"-"+g.name.replace(/\s/g,"")+".png";
    a.href = c.toDataURL("image/png");
    a.click();
  }

  // Bütün qonaqlar üçün biri-biri ardınca WhatsApp göndər
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:500,background:"rgba(0,0,0,.95)",display:"flex",flexDirection:"column",overflowY:"auto"}}>
      {/* Header */}
      <div style={{background:"#0a0700",borderBottom:"1px solid rgba(201,168,76,.2)",padding:"12px 16px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
        <span style={{fontSize:14,fontWeight:700,color:"#c9a84c"}}>🎊 Dəvətnamə — Masa {tbl.id} ({guests.length} qonaq)</span>
        <button onClick={onClose} style={{background:"none",border:"none",color:"#9a8060",fontSize:20,cursor:"pointer"}}>✕</button>
      </div>

      {/* Şablon seçimi */}
      <div style={{padding:"10px 16px",background:"#0a0700",borderBottom:"1px solid rgba(201,168,76,.1)",flexShrink:0}}>
        <div style={{fontSize:10,color:"rgba(255,255,255,.35)",marginBottom:6}}>Şablon:</div>
        <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:2}}>
          {DEVETNAME_SHABLONLAR.map(s=>(
            <button key={s.id} onClick={()=>setShablon(s)}
              style={{padding:"5px 12px",borderRadius:20,border:"1px solid "+(shablon.id===s.id?s.accent:"rgba(255,255,255,.12)"),background:shablon.id===s.id?"rgba(201,168,76,.1)":"transparent",color:shablon.id===s.id?s.accent:"rgba(255,255,255,.35)",fontSize:10,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0}}>
              {s.ad}
            </button>
          ))}
        </div>
      ...
      {/* Qonaq siyahısı + preview */}
      <div style={{display:"flex",flex:1,overflow:"hidden",minHeight:0}}>

        {/* Sol — qonaq siyahısı */}
        <div style={{width:"42%",borderRight:"1px solid rgba(201,168,76,.1)",overflowY:"auto",background:"#070502"}}>
          <div style={{padding:"8px 10px",fontSize:10,color:"rgba(255,255,255,.3)"}}>Qonaqlar — preview üçün seç:</div>
          {guests.map((g,i)=>{
            const sc=g.gender==="kishi"?"#7aade8":g.gender==="qadin"?"#e87aad":"#c9a84c";
            const hasPhone=!!(g.phone||"").replace(/\D/g,"");
            return (
              <div key={g.id||i} onClick={()=>setSelIdx(i)}
                style={{padding:"10px",cursor:"pointer",borderBottom:"1px solid rgba(255,255,255,.04)",background:selIdx===i?"rgba(201,168,76,.1)":"transparent",transition:"background .15s"}}>
                <div style={{display:"flex",alignItems:"center",gap:8}}>
                  <div style={{width:28,height:28,borderRadius:"50%",background:sc+"22",border:"1px solid "+sc+"44",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:sc,flexShrink:0}}>
                    {g.name[0]||"?"}
                  </div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:12,fontWeight:600,color:"#f2e8d0",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{g.name}</div>
                    <div style={{fontSize:10,color:hasPhone?"rgba(37,211,102,.6)":"rgba(255,80,80,.5)"}}>{hasPhone?"📱 "+g.phone:"nömrə yox"}</div>
                  </div>
                </div>
                {/* Tək göndər düymələri */}
                {selIdx===i&&(
                  <div style={{display:"flex",gap:5,marginTop:8}}>
                    <button onClick={e=>{e.stopPropagation();downloadOne(g);}}
                      style={{flex:1,padding:"5px",borderRadius:7,border:"1px solid rgba(201,168,76,.3)",background:"rgba(201,168,76,.08)",color:"#c9a84c",fontSize:10,fontWeight:700,cursor:"pointer"}}>
                      ⬇️ PNG
                    </button>
                    <button onClick={e=>{e.stopPropagation();sendOneWA(g);}}
                      style={{flex:1,padding:"5px",borderRadius:7,border:"none",background:"rgba(37,211,102,.2)",color:"#25d366",fontSize:10,fontWeight:700,cursor:"pointer"}}>
                      📱 WA
                    </button>
                  </div>
                )}
        ...
        {/* Sağ — canvas preview */}
        <div style={{flex:1,overflowY:"auto",background:"#060402",display:"flex",justifyContent:"center",alignItems:"flex-start",padding:"12px 8px"}}>
          <canvas ref={canvasRef} style={{width:"100%",maxWidth:280,borderRadius:10,display:"block"}}/>
        </div>
      </div>

      {/* Aşağı — Hamısını göndər */}
      <div style={{padding:"10px 14px 32px",background:"#0a0700",borderTop:"1px solid rgba(201,168,76,.12)",flexShrink:0}}>
        {sending?(
          <div style={{textAlign:"center",padding:"12px"}}>
            <div style={{fontSize:13,color:"#25d366",fontWeight:700,marginBottom:4}}>
              📤 Göndərilir... {sentCount}/{guests.filter(g=>(g.phone||"").replace(/\D/g,"").length>=7).length}
            </div>
            <div style={{fontSize:11,color:"rgba(255,255,255,.4)"}}>Hər qonaq üçün ayrı dəvətnamə...</div>
          </div>
        ):(
          <button onClick={sendAllWA}
            style={{width:"100%",padding:"14px",borderRadius:12,border:"none",background:"linear-gradient(90deg,rgba(37,211,102,.5),rgba(37,211,102,.25))",color:"#25d366",fontSize:14,fontWeight:800,cursor:"pointer"}}>
            📱 Hamısına Göndər ({guests.filter(g=>(g.phone||"").replace(/\D/g,"").length>=7).length} nömrə)
          </button>
        )}
      </div>
    </div>
...
export default function App(){
  ...
  const [inviteTab, setInviteTab] = useState("metn");
...
.finger{animation:fingerpoint 1s ease forwards;}\n`;
  ...
  return (
    <div>
      ...
      {/* DƏVƏTNAMƏ PANEL */}
      {inviteOpen&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.7)",zIndex:100}} onClick={()=>setInviteOpen(false)}>
          <div style={{position:"absolute",left:0,right:0,bottom:0,maxHeight:"90vh",background:"#080604",borderTop:"1px solid rgba(201,168,76,.25)",borderRadius:"20px 20px 0 0",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
            <div style={{display:"flex",justifyContent:"center",padding:"10px 0 4px"}}>
              <div style={{width:36,height:4,borderRadius:2,background:"rgba(201,168,76,.25)"}}/>
            </div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"0 16px 10px"}}>
              <div style={{fontFamily:"'Playfair Display',serif",color:"#c9a84c",fontSize:16}}>📨 Dəvətnamə</div>
              <button style={{background:"transparent",border:"none",color:"#9a8060",fontSize:18,cursor:"pointer"}} onClick={()=>setInviteOpen(false)}>✕</button>
            </div>
            <div style={{flex:1,overflowY:"auto",padding:"0 14px 32px"}}>
              <DevetNamePanel
                ...
                obData={obData}
                hall={hall}
                initialTab={inviteTab||"oz"}
                devetData={devetData}
                onSave={(data)=>{ setDevetData(data); setInviteOpen(false); }}
              />
            </div>
          </div>
        </div>

...
export default function AuthScreen({ supabase, onAuthenticated }){
  ...
  async function sendCode(){
    ...
    if(mode==="register" && !firstName.trim()){ setErr("Adınızı yazın"); return; }
    setBusy(true); setErr("");
    try{
      const opts = mode==="register"
  ...
  async function verifyCode(){
    ...
    try{
      ...
      if(fullName && !(data.user&&data.user.user_metadata&&data.user.user_metadata.full_name)){
    ...
    <div style={{position:"fixed",inset:0,zIndex:1000,
      ...
      {phase==="form"&&(
        <div style={{width:"100%",maxWidth:340,animation:"authFadeIn .4s ease"}}>
          ...
          {step==="form"&&(
            <>
              {/* Giriş / Qeydiyyat keçidi */}
              <div style={{display:"flex",background:"rgba(255,255,255,.5)",borderRadius:14,padding:4,marginBottom:18,border:"1px solid rgba(255,255,255,.6)"}}>
                <button onClick={()=>{setMode("login");setErr("");}}
                  style={{flex:1,padding:"10px",borderRadius:10,border:"none",cursor:"pointer",
                    background:mode==="login"?"#211A16":"transparent",color:mode==="login"?"#fff":"#6B6259",
                    fontSize:13,fontWeight:700,transition:"all .2s"}}>
                  Giriş
                </button>
                <button onClick={()=>{setMode("register");setErr("");}}
                  style={{flex:1,padding:"10px",borderRadius:10,border:"none",cursor:"pointer",
                    background:mode==="register"?"#211A16":"transparent",color:mode==="register"?"#fff":"#6B6259",
                    fontSize:13,fontWeight:700,transition:"all .2s"}}>
                  Qeydiyyat
                </button>
              </div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                ...
                {mode==="register"&&(
                  <div style={{display:"flex",gap:8}}>
                    <input value={firstName} onChange={e=>setFirstName(e.target.value)} placeholder="Ad" style={{...inp,flex:1}}/>
                    <input value={lastName} onChange={e=>setLastName(e.target.value)} placeholder="Soyad" style={{...inp,flex:1}}/>
                  </div>
                )}
                ...
                <div style={{textAlign:"center",fontSize:10,color:"rgba(33,26,22,.4)",marginTop:4}}>
